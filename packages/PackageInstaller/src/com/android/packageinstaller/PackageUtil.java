...
public class PackageUtil {
    ...
    static final class AppSnippet implements Parcelable {
        ...
        private Bitmap getBitmapFromDrawable(Drawable drawable) {
            ...
            // to few 100 KiB
            final int MaxSize = 250; // 250 KB at 4 bytes per pixel

            int w = origW;
            int h = origH;
            if (Math.max(origW, origH) > MaxSize) {
                double ratio = Math.min((double) MaxSize / origW, (double) MaxSize / origH);
                w = (int) (ratio * w);
                h = (int) (ratio * h);
                Log.d("AppSnippet", "getBitmapFromDrawable: downscaling, " +
                        "origW " + origW + " origH " + origH + " scaleRatio " + ratio +
                        " label " + label);
            }
